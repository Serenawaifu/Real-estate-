...
curl -s https://dgrealtors.in/health | jq '.'
...
curl -s https://api.dgrealtors.in/metrics/daily | jq '.'
...
openssl s_client -connect dgrealtors.in:443 -servername dgrealtors.in < /dev/null 2>/dev/null | \
...
class WeeklyMaintenance {
  ...
  async analyzePerformance() {
    ...
    const lighthouse = await execPromise(`
      lighthouse https://dgrealtors.in \
        --output=json \
  ...
  async securityAudit() {
    ...
    // 2. Security headers
    const headers = await fetch('https://dgrealtors.in', {
      method: 'HEAD'
    });
    audits.push({
      type: 'headers',
    ...
    const sslScan = await fetch(
      `https://api.ssllabs.com/api/v3/analyze?host=dgrealtors.in`
    );
    audits.push({
      type: 'ssl',
...
CERT_EXPIRY=$(echo | openssl s_client -connect dgrealtors.in:443 2>/dev/null | \
...
class ApplicationUpdater {
  ...
  async downloadNewVersion() {
    ...
    const downloadUrl = this.options.version
      ? `https://releases.dgrealtors.in/v${this.options.version}.tar.gz`
      : 'https://releases.dgrealtors.in/latest.tar.gz';
...
class SSLManager {
  constructor() {
    this.domains = ['dgrealtors.in', 'www.dgrealtors.in'];
    this.email = 'admin@dgrealtors.in';
    this.certPath = '/etc/letsencrypt/live/dgrealtors.in';
  }
  ...
  async verifyCertificate() {
    ...
    // Test HTTPS
    const response = await fetch('https://dgrealtors.in', {
      timeout: 5000
    });
  ...
  async checkSSLRating() {
    const response = await fetch(
      `https://api.ssllabs.com/api/v3/analyze?host=dgrealtors.in&publish=off&all=done`
    );
...
class PerformanceMonitor {
  ...
  async getNetworkMetrics() {
    ...
    // Response times
    const endpoints = [
      { name: 'homepage', url: 'https://dgrealtors.in' },
      { name: 'api', url: 'https://api.dgrealtors.in/health' },
      { name: 'cdn', url: 'https://cdn.dgrealtors.in/health.txt' }
    ];
...
curl -s -I https://dgrealtors.in > $AUDIT_DIR/security-headers.txt

# 3. SSL/TLS Security
echo "Checking SSL/TLS security..."
testssl --jsonfile $AUDIT_DIR/testssl.json https://dgrealtors.in
...
if [ -f /opt/zap/zap.sh ]; then
  /opt/zap/zap.sh -cmd \
    -quickurl https://api.dgrealtors.in \
...
# Check network
ping -c 3 api.dgrealtors.in
curl -I https://dgrealtors.in
...
class HealthCheckSystem {
  constructor() {
    this.checks = [
      ...
      {
        name: 'api',
        url: 'https://api.dgrealtors.in/health',
        expectedStatus: 200,
        timeout: 3000,
        critical: true
      },
      {
        name: 'cdn',
        url: 'https://cdn.dgrealtors.in/health.txt',
        expectedStatus: 200,
        timeout: 3000,
        critical: false
      },
